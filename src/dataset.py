...
class RawCodeDataset(Dataset):
        ...
        if subsample:
            ...
            try:
                ...
                full_dataset = full_dataset.cast_column(
                    "stratify_col", ClassLabel(names=list(set(full_dataset["stratify_col"])))
                )
            ...
            if len(full_dataset) > sample_size:
                ...
                self.raw_dataset = full_dataset.train_test_split(
                    test_size=proportion, shuffle=True, seed=10, stratify_by_column="stratify_col"
                )["test"]
    ...
    def __getitem__(self, idx):
        ...
        return {"code": item["code"], "label": item["label"], "language": item["language"]}
...
def create_stratification_column(example):
    ...
    """
    return {"stratify_col": f"{example['label']}_{example['language']}"}
